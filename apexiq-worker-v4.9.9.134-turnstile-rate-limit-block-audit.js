...
const VERSION = "4.9.9.134-worker-turnstile-rate-limit-block-audit";
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      const betaAccessRoute = await routeBetaAccess(request, env, origin, url, ctx);
      ...
      if (url.pathname === "/api/health") {
        return json({
          ...
          pushNotificationsConfigured: !!(env.APEXIQ_VAPID_PUBLIC_KEY && env.APEXIQ_VAPID_PRIVATE_JWK),
          requestSecurityConfigured: !!(
            env.APEXIQ_TURNSTILE_SITE_KEY &&
            env.APEXIQ_TURNSTILE_SECRET_KEY &&
            (env.APEXIQ_SECURITY_PEPPER || env.APEXIQ_ADMIN_SECRET)
          ),
          renderedRecovery: browserReady(env),
          rollbackSafe: true,
          stableBase: "v4.6.8",
          endpoints: ["/api/player","/api/ranked-fast","/api/ranked","/api/meta","/api/beta/verify","/api/beta/session","/api/beta/admin/login","/api/beta/founder-status","/api/beta/security-config","/api/beta/push-public-key","/api/beta/request-access","/api/beta/request-status","/api/beta/request-recover","/api/beta/admin/users","/api/beta/admin/set-founder","/api/beta/admin/requests","/api/beta/admin/security-status","/api/beta/admin/blocks","/api/beta/admin/unblock","/api/beta/admin/request-block","/api/beta/admin/push-status","/api/beta/admin/push-subscribe","/api/beta/admin/push-unsubscribe","/api/beta/admin/push-test","/api/beta/admin/request-approve","/api/beta/admin/request-delete","/api/beta/admin/logout","/api/rendered-profile","/api/badge-scan","/api/rendered-ranked","/api/doctor","/api/als-test","/api/switch-ranked","/api/health"]
        }, origin);
      }
...
const BETA_REQUEST_MAX_CLAIMS = 4;
const BETA_PUSH_PREFIX = "beta_admin_push:";
const BETA_REQUEST_AUDIT_PREFIX = "beta_request_audit:";
const BETA_REQUEST_BLOCK_PREFIX = "beta_request_block:";
const BETA_REQUEST_RATE_PREFIX = "beta_request_rate:";
const BETA_REQUEST_AUDIT_TTL = 60 * 60 * 24 * 30;
const BETA_REQUEST_BLOCK_TTL = 60 * 60 * 24 * 180;
const BETA_REQUEST_LIMITS = Object.freeze({
  ip: { limit: 4, windowSeconds: 60 * 60 },
  device: { limit: 3, windowSeconds: 60 * 60 * 6 },
  username: { limit: 2, windowSeconds: 60 * 60 * 24 }
});

async function routeBetaAccess(request, env, origin, url, ctx) {
  ...
  try {
    ...
    if (url.pathname === "/api/beta/security-config" && request.method === "GET") {
      const siteKey = String(env.APEXIQ_TURNSTILE_SITE_KEY || "").trim();
      const configured = !!(
        siteKey &&
        env.APEXIQ_TURNSTILE_SECRET_KEY &&
        (env.APEXIQ_SECURITY_PEPPER || env.APEXIQ_ADMIN_SECRET)
      ...
      return jsonPrivate({
        ...
        turnstileAction: "beta_request",
        requestLimits: BETA_REQUEST_LIMITS,
        error: configured ? undefined : "ApexIQ request security is not configured."
      }, origin, configured ? 200 : 503);
    }

    if (url.pathname === "/api/beta/push-public-key" && request.method === "GET") {
      const publicKey = String(env.APEXIQ_VAPID_PUBLIC_KEY || "").trim();
      return jsonPrivate({
        ok: !!publicKey,
        publicKey,
        configured: !!(publicKey && env.APEXIQ_VAPID_PRIVATE_JWK),
        error: publicKey ? undefined : "Web Push is not configured on this Worker."
      }, origin, publicKey ? 200 : 503);
    }
    ...
    if (url.pathname === "/api/beta/request-access" && request.method === "POST") {
      const data = await betaRequestAccess(kv, env, body, request);
      if (data.ok && data.acceptedForReview !== false) {
        const task = notifyAdminPushSubscriptions(kv, env, {
          requestId: data.requestId || data.id || "",
          apexUsername: data.apexUsername || body.apexUsername || body.username || "",
          platform: data.platform || body.platform || "Auto"
        });
        if (ctx && typeof ctx.waitUntil === "function") ctx.waitUntil(task);
        else await task;
      }
      return jsonPrivate(data, origin, data.ok ? 200 : (data.statusCode || 400));
    }

    if (url.pathname === "/api/beta/request-status" && request.method === "POST") {
      const data = await betaRequestStatus(kv, body);
      return jsonPrivate(data, origin, data.ok ? 200 : (data.statusCode || 403));
    }

    if (url.pathname === "/api/beta/request-recover" && request.method === "POST") {
      const data = await betaRequestRecover(kv, body);
      return jsonPrivate(data, origin, data.ok ? 200 : (data.statusCode || 403));
    }
    ...
    if (url.pathname.startsWith("/api/beta/admin/")) {
      ...
      if (url.pathname === "/api/beta/admin/push-status" && request.method === "GET") {
        return jsonPrivate(await betaAdminPushStatus(kv, env), origin, 200);
      }
      if (url.pathname === "/api/beta/admin/push-subscribe" && request.method === "POST") {
        return jsonPrivate(await betaAdminPushSubscribe(kv, body, admin), origin, 200);
      }
      if (url.pathname === "/api/beta/admin/push-unsubscribe" && request.method === "POST") {
        return jsonPrivate(await betaAdminPushUnsubscribe(kv, body, admin), origin, 200);
      }
      if (url.pathname === "/api/beta/admin/push-test" && request.method === "POST") {
        return jsonPrivate(await betaAdminPushTest(kv, env, admin), origin, 200);
      }
      if (url.pathname === "/api/beta/admin/requests" && request.method === "GET") {
        return jsonPrivate(await betaAdminRequests(kv), origin, 200);
      }
      if (url.pathname === "/api/beta/admin/security-status" && request.method === "GET") {
        return jsonPrivate(await betaAdminSecurityStatus(kv, env), origin, 200);
      }
      if (url.pathname === "/api/beta/admin/blocks" && request.method === "GET") {
        return jsonPrivate(await betaAdminBlocks(kv), origin, 200);
      }
      if (url.pathname === "/api/beta/admin/unblock" && request.method === "POST") {
        return jsonPrivate(await betaAdminUnblock(kv, body, admin), origin, 200);
      }
      if (url.pathname === "/api/beta/admin/request-block" && request.method === "POST") {
        return jsonPrivate(await betaAdminRequestBlock(kv, body, admin), origin, 200);
      }
      if (url.pathname === "/api/beta/admin/request-approve" && request.method === "POST") {
        return jsonPrivate(await betaAdminRequestApprove(kv, body, admin), origin, 200);
      }
...
function pushBase64UrlEncode(input) {
  const bytes = input instanceof Uint8Array ? input : new Uint8Array(input);
  let binary = "";
  for (let i = 0; i < bytes.length; i++) binary += String.fromCharCode(bytes[i]);
  return btoa(binary).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/g, "");
}
function pushBase64UrlDecode(value) {
...
async function pushSha256(value) {
  return new Uint8Array(await crypto.subtle.digest("SHA-256", pushText(value)));
}
async function pushSubscriptionKey(endpoint) {
  return BETA_PUSH_PREFIX + pushBase64UrlEncode(await pushSha256(endpoint));
}
function normalizePushSubscription(input) {
  const subscription = input && input.subscription ? input.subscription : input;
  const endpoint = String(subscription && subscription.endpoint || "").trim();
  const p256dh = String(subscription && subscription.keys && subscription.keys.p256dh || "").trim();
  const auth = String(subscription && subscription.keys && subscription.keys.auth || "").trim();
...
async function vapidPrivateJwk(env) {
  const raw = String(env.APEXIQ_VAPID_PRIVATE_JWK || "").trim();
  if (!raw) throw new Error("APEXIQ_VAPID_PRIVATE_JWK is not configured.");
  let jwk;
  try { jwk = JSON.parse(raw); }
  catch (_) { throw new Error("APEXIQ_VAPID_PRIVATE_JWK is not valid JSON."); }
  return jwk;
}
async function createVapidAuthorization(endpoint, env) {
  const publicKey = String(env.APEXIQ_VAPID_PUBLIC_KEY || "").trim();
  if (!publicKey) throw new Error("APEXIQ_VAPID_PUBLIC_KEY is not configured.");
  const privateJwk = await vapidPrivateJwk(env);
  const audience = new URL(endpoint).origin;
  const subject = String(env.APEXIQ_VAPID_SUBJECT || "mailto:coyemicthell@gmail.com").trim();
  const now = Math.floor(Date.now() / 1000);
  const header = pushBase64UrlEncode(pushText(JSON.stringify({ typ: "JWT", alg: "ES256" })));
  const payload = pushBase64UrlEncode(pushText(JSON.stringify({
    aud: audience,
    exp: now + 12 * 60 * 60,
    sub: subject
  })));
  ...
  return `vapid t=${unsigned}.${pushBase64UrlEncode(signature)}, k=${publicKey}`;
}
...
async function listAdminPushRecords(kv) {
  const records = [];
  let cursor;
  do {
    const listed = await kv.list({ prefix: BETA_PUSH_PREFIX, cursor, limit: 1000 });
    cursor = listed.cursor;
    for (const item of listed.keys || []) {
      const record = await kvGetJSON(kv, item.name);
      if (record && record.subscription && record.subscription.endpoint) {
        records.push({ key: item.name, record });
      }
    }
  } while (cursor);
  return records;
}
async function deliverAdminPush(kv, env) {
  const records = await listAdminPushRecords(kv);
  let delivered = 0;
  let removed = 0;
  const errors = [];
  await Promise.all(records.map(async ({ key, record }) => {
    try {
      const response = await sendEmptyWebPush(record.subscription, env);
      ...
      if (!response.ok) {
        errors.push(`${response.status} ${response.statusText || "Push service error"}`.trim());
        return;
      }
      delivered += 1;
      record.lastPushAt = isoNow();
      record.lastPushStatus = response.status;
      await kvPutJSON(kv, key, record);
    } catch (error) {
      errors.push(String(error && error.message ? error.message : error));
    }
  }));
  return { ok: errors.length === 0 || delivered > 0, delivered, removed, total: records.length, errors };
}
async function notifyAdminPushSubscriptions(kv, env, requestInfo) {
  if (!(env.APEXIQ_VAPID_PUBLIC_KEY && env.APEXIQ_VAPID_PRIVATE_JWK)) {
    return { ok: false, skipped: true, reason: "Web Push is not configured." };
  }
  const result = await deliverAdminPush(kv, env);
  return { ...result, requestId: requestInfo && requestInfo.requestId || "" };
}
async function betaAdminPushStatus(kv, env) {
  const records = await listAdminPushRecords(kv);
  return {
    ok: true,
    configured: !!(env.APEXIQ_VAPID_PUBLIC_KEY && env.APEXIQ_VAPID_PRIVATE_JWK),
    subscriptions: records.length,
    devices: records.map(({ record }) => ({
      endpoint: record.subscription.endpoint,
      label: record.label || "ApexIQ Admin",
...
async function betaAdminPushSubscribe(kv, body, admin) {
  const subscription = normalizePushSubscription(body);
  if (!subscription) return { ok: false, error: "Invalid Push API subscription." };
  const key = await pushSubscriptionKey(subscription.endpoint);
  const prior = await kvGetJSON(kv, key);
  const now = isoNow();
  const record = {
    subscription,
    label: String(body.label || prior && prior.label || "ApexIQ Admin").trim().slice(0, 100),
    createdAt: prior && prior.createdAt || now,
    updatedAt: now,
    adminEmail: admin.email
  };
  await kvPutJSON(kv, key, record);
  return { ok: true, subscribed: true, endpoint: subscription.endpoint, record: {
    label: record.label,
    createdAt: record.createdAt,
    updatedAt: record.updatedAt
  }};
}
async function betaAdminPushUnsubscribe(kv, body, admin) {
  const endpoint = String(body.endpoint || body.subscription && body.subscription.endpoint || "").trim();
  if (!endpoint) return { ok: false, error: "Missing push endpoint." };
  await kv.delete(await pushSubscriptionKey(endpoint));
  return { ok: true, unsubscribed: true, updatedBy: admin.email };
}
async function betaAdminPushTest(kv, env, admin) {
  const result = await deliverAdminPush(kv, env);
  return { ...result, testedBy: admin.email };
}
...
function requestSecurityPepper(env) {
  return String(env.APEXIQ_SECURITY_PEPPER || env.APEXIQ_ADMIN_SECRET || "").trim();
}
function cleanRequestDeviceId(input) {
  const value = String(input || "").trim();
  return /^[A-Za-z0-9_-]{16,120}$/.test(value) ? value : "";
}
...
async function protectedRequestHash(env, label, value) {
  const pepper = requestSecurityPepper(env);
  if (!pepper) throw new Error("APEXIQ_SECURITY_PEPPER is not configured.");
  return sha256Hex(`${pepper}|${label}|${String(value || "")}`);
}
function requestBlockKey(kind, valueHash) {
  return `${BETA_REQUEST_BLOCK_PREFIX}${kind}:${valueHash}`;
}
function requestRateKey(kind, valueHash, windowSeconds, now = Date.now()) {
  const bucket = Math.floor(now / 1000 / windowSeconds);
  return `${BETA_REQUEST_RATE_PREFIX}${kind}:${valueHash}:${bucket}`;
}
async function consumeRequestLimit(kv, kind, valueHash, config) {
  if (!valueHash) return { ok: true, remaining: config.limit };
  const now = Date.now();
  const key = requestRateKey(kind, valueHash, config.windowSeconds, now);
  const existing = await kvGetJSON(kv, key);
  const count = Number(existing && existing.count || 0);
  if (count >= config.limit) {
    const bucketEnd = (Math.floor(now / 1000 / config.windowSeconds) + 1) * config.windowSeconds;
    return {
      ok: false,
      retryAfter: Math.max(1, bucketEnd - Math.floor(now / 1000)),
      limit: config.limit,
  ...
  await kvPutJSON(kv, key, next, { expirationTtl: config.windowSeconds + 120 });
  return {
    ok: true,
...
async function verifyTurnstileRequest(env, tokenValue, request) {
  const secret = String(env.APEXIQ_TURNSTILE_SECRET_KEY || "").trim();
  const siteKey = String(env.APEXIQ_TURNSTILE_SITE_KEY || "").trim();
  const pepper = requestSecurityPepper(env);
  if (!secret || !siteKey || !pepper) {
    return {
      ok: false,
      statusCode: 503,
      error: "ApexIQ request security is not configured yet."
    };
  }
  const turnstileToken = String(tokenValue || "").trim();
  if (!turnstileToken) return { ok: false, statusCode: 400, error: "Complete the security check." };
  ...
  form.set("idempotency_key", crypto.randomUUID ? crypto.randomUUID() : token("ts_"));
  ...
  try {
    ...
    return { ok: false, statusCode: 503, error: "The security check could not be verified. Try again." };
  }

  let result;
  try { result = await response.json(); }
  catch (_) { return { ok: false, statusCode: 503, error: "The security service returned an invalid response." }; }

  if (!response.ok || !result.success) {
    return {
      ok: false,
      statusCode: 403,
      error: "Security verification failed. Refresh the challenge and try again.",
      errorCodes: Array.isArray(result["error-codes"]) ? result["error-codes"] : []
    };
  }

  const expectedHostname = String(env.APEXIQ_TURNSTILE_HOSTNAME || "imissanyway.github.io").trim().toLowerCase();
  const actualHostname = String(result.hostname || "").trim().toLowerCase();
  if (expectedHostname && actualHostname !== expectedHostname) {
    return { ok: false, statusCode: 403, error: "Security verification hostname did not match ApexIQ." };
  }
  if (String(result.action || "") !== "beta_request") {
    return { ok: false, statusCode: 403, error: "Security verification action did not match the request form." };
  }
...
async function requestBlockStatus(kv, identifiers) {
  const matches = [];
  for (const [kind, valueHash] of Object.entries(identifiers || {})) {
    if (!valueHash) continue;
    const storageKind = kind === "legacyUsername" ? "username" : kind;
    const key = requestBlockKey(storageKind, valueHash);
    const record = await kvGetJSON(kv, key);
    if (record) matches.push({ key, kind, record });
  }
  return { blocked: matches.length > 0, matches };
}
async function putRequestBlock(kv, kind, valueHash, display, admin, reason) {
  if (!valueHash) return null;
  const now = Date.now();
  const record = {
    key: requestBlockKey(kind, valueHash),
    kind,
    valueHash,
    display: String(display || "").slice(0, 140),
    reason: String(reason || "Blocked from an access request").slice(0, 300),
    createdAt: new Date(now).toISOString(),
    expiresAt: new Date(now + BETA_REQUEST_BLOCK_TTL * 1000).toISOString(),
    createdBy: admin.email
  };
  await kvPutJSON(kv, record.key, record, { expirationTtl: BETA_REQUEST_BLOCK_TTL });
  return record;
}
function sanitizedRequestAudit(record, admin, action, reason) {
  const security = record && record.security || {};
  return {
    id: record.id || "",
    apexUsername: record.apexUsername || "",
    apexUsernameNorm: record.apexUsernameNorm || "",
    platform: record.platform || "Auto",
    name: record.name || "",
    mainLegend: record.mainLegend || "",
    ...
    security: {
      ...
      colo: security.colo || "",
      turnstileVerified: security.turnstileVerified === true,
      turnstileHostname: security.turnstileHostname || "",
      turnstileAction: security.turnstileAction || ""
    }
  };
}
async function archiveRequestAudit(kv, record, admin, action, reason) {
  if (!record) return null;
  const audit = sanitizedRequestAudit(record, admin, action, reason);
  const suffix = `${Date.now()}_${randomPart(6)}`;
  await kvPutJSON(
    kv,
    `${BETA_REQUEST_AUDIT_PREFIX}${record.id}:${suffix}`,
    audit,
    { expirationTtl: BETA_REQUEST_AUDIT_TTL }
  );
  return audit;
}
...
function cleanRequestId(input) {
  return String(input || "").trim().replace(/[^A-Za-z0-9_-]/g, "").slice(0, 100);
}
...
function requestClaimHashes(record) {
  const hashes = Array.isArray(record && record.claimTokenHashes) ? record.claimTokenHashes : [];
  if (record && record.claimTokenHash && !hashes.includes(record.claimTokenHash)) hashes.push(record.claimTokenHash);
  return hashes.filter(Boolean).slice(-BETA_REQUEST_MAX_CLAIMS);
}
async function appendRequestClaim(record, rawClaimToken) {
  const hash = await sha256Hex(rawClaimToken);
  const hashes = requestClaimHashes(record).filter(existing => existing !== hash);
  hashes.push(hash);
  record.claimTokenHashes = hashes.slice(-BETA_REQUEST_MAX_CLAIMS);
  delete record.claimTokenHash;
}
async function hasRequestClaim(record, rawClaimToken) {
  if (!rawClaimToken) return false;
  const wanted = await sha256Hex(rawClaimToken);
  return requestClaimHashes(record).some(hash => secretEqual(hash, wanted));
}
async function hasRecoveryCode(record, rawRecoveryCode) {
  if (!record || !record.recoveryCodeHash || !rawRecoveryCode) return false;
  const wanted = await sha256Hex(cleanRecoveryCode(rawRecoveryCode));
  return secretEqual(record.recoveryCodeHash, wanted);
}
function publicBetaRequest(record, includeCode = false) {
  return {
    id: record.id,
    apexUsername: record.apexUsername || "",
    platform: record.platform || "Auto",
    ...
    security: {
      turnstileVerified: !!(record.security && record.security.turnstileVerified),
      country: record.security && record.security.country || "",
      hasIpHash: !!(record.security && record.security.ipHash),
      hasDeviceHash: !!(record.security && record.security.deviceHash)
    },
    approvedCode: includeCode && record.approvedCode ? record.approvedCode : undefined
  };
}
async function deliverRequestStatus(kv, key, record) {
  const response = publicBetaRequest(record, record.status === "approved");
  response.ok = true;
  response.pending = ["new", "reviewed"].includes(record.status);
  if (record.status === "approved" && record.approvedCode) {
    const now = isoNow();
    record.firstDeliveredAt = record.firstDeliveredAt || now;
    record.lastDeliveredAt = now;
    record.deliveryCount = Number(record.deliveryCount || 0) + 1;
    record.updatedAt = now;
    await kvPutJSON(kv, key, record);
    response.accessKey = record.approvedCode;
    response.assignedUsername = record.apexUsername || "";
    response.message = "Your ApexIQ beta access key is ready.";
    response.firstDeliveredAt = record.firstDeliveredAt;
    response.lastDeliveredAt = record.lastDeliveredAt;
    response.deliveryCount = record.deliveryCount;
  } else if (record.status === "denied") {
    response.message = "This beta request was not accepted.";
  } else if (record.status === "reviewed") {
    response.message = "Your request is being reviewed.";
  } else {
    response.message = "Your request is waiting for review.";
  }
  return response;
}
async function findCodeCreatedForRequest(kv, requestId) {
  let cursor;
  do {
    const listed = await kv.list({ prefix: BETA_CODE_PREFIX, cursor, limit: 250 });
    cursor = listed.cursor;
    for (const item of listed.keys || []) {
      ...
      if (notes.includes(`request ${requestId}`) || notes.includes(`Request ${requestId}`)) {
        return record.code || item.name.slice(BETA_CODE_PREFIX.length);
      }
    }
  } while (cursor);
  return "";
...
async function betaRequestAccess(kv, env, body, request) {
  const apexUsername = cleanUsername(body.apexUsername || body.username);
  const usernameNorm = normUsername(apexUsername);
  if (!apexUsername || usernameNorm.length < 3) {
    return { ok: false, statusCode: 400, error: "Enter your exact Apex username." };
  }
  if (apexUsername.length > 80) {
    return { ok: false, statusCode: 400, error: "That Apex username is too long." };
  }
  if (!body.agreedNoShare) {
    return { ok: false, statusCode: 400, error: "Confirm that you will not share the access key." };
  }

  const turnstile = await verifyTurnstileRequest(env, body.turnstileToken, request);
  if (!turnstile.ok) return turnstile;

  const ip = requestClientIp(request);
  const requestDeviceId = cleanRequestDeviceId(body.requestDeviceId);
  if (!requestDeviceId) {
    return { ok: false, statusCode: 400, error: "Could not verify this browser. Refresh and try again." };
  }

  const userAgent = String(request.headers.get("User-Agent") || "").slice(0, 300);
  const ipHash = ip ? await protectedRequestHash(env, "ip", ip) : "";
  const deviceHash = await protectedRequestHash(env, "device", `${requestDeviceId}|${userAgent}`);
  const usernameHash = await protectedRequestHash(env, "username", usernameNorm);

  const blocked = await requestBlockStatus(kv, {
    username: usernameHash,
    legacyUsername: await sha256Hex(`legacy-username|${usernameNorm}`),
    ip: ipHash,
    device: deviceHash
  });
  if (blocked.blocked) {
    return {
      ok: false,
      statusCode: 403,
      error: "Requests from this account or browser are blocked."
    };
  }

  const limits = {
    ip: await consumeRequestLimit(kv, "ip", ipHash, BETA_REQUEST_LIMITS.ip),
    device: await consumeRequestLimit(kv, "device", deviceHash, BETA_REQUEST_LIMITS.device),
    username: await consumeRequestLimit(kv, "username", usernameHash, BETA_REQUEST_LIMITS.username)
  };
  const limited = Object.entries(limits).find(([, result]) => !result.ok);
  if (limited) {
    const [kind, result] = limited;
    return {
      ok: false,
      statusCode: 429,
      retryAfter: result.retryAfter,
      error: `Too many access requests from this ${kind}. Try again later.`
    };
  }

  const now = isoNow();
  const id = `${Date.now().toString(36)}_${randomPart(8)}`;
  const claimToken = token("rq_");
  const recoveryCode = `RQ-${randomPart(4)}-${randomPart(4)}-${randomPart(4)}`;
  const record = {
    id,
    apexUsername,
    apexUsernameNorm: usernameNorm,
    ...
    createdAt: now,
    updatedAt: now,
    recoveryCodeHash: await sha256Hex(cleanRecoveryCode(recoveryCode)),
    recoveryHint: recoveryCode.slice(-4),
    claimTokenHashes: [],
    security: {
      ipHash,
      deviceHash,
      usernameHash,
      country: String(request.cf && request.cf.country || request.headers.get("CF-IPCountry") || "").slice(0, 8),
      colo: String(request.cf && request.cf.colo || "").slice(0, 20),
      userAgentHash: await protectedRequestHash(env, "ua", userAgent),
      turnstileVerified: true,
      turnstileHostname: turnstile.hostname,
      turnstileAction: turnstile.action,
      turnstileChallengeTs: turnstile.challengeTs,
      requestSecurityVersion: "v134"
    }
  };
  await appendRequestClaim(record, claimToken);
  ...
  return {
    ...
    createdAt: now,
    message: "Request sent. This browser will retrieve the key automatically after approval."
  };
}

async function betaRequestStatus(kv, body) {
  const id = cleanRequestId(body.id || body.requestId);
  const claimToken = String(body.claimToken || "").trim();
  if (!id || !claimToken) return { ok: false, statusCode: 400, error: "Missing request claim." };

  const key = BETA_REQUEST_PREFIX + id;
  const record = await kvGetJSON(kv, key);
  if (!record) return { ok: false, statusCode: 404, error: "Request not found." };
  if (!(await hasRequestClaim(record, claimToken))) {
    return { ok: false, statusCode: 403, error: "Request claim is not valid on this browser." };
  }
  return deliverRequestStatus(kv, key, record);
}

async function betaRequestRecover(kv, body) {
  const id = cleanRequestId(body.id || body.requestId);
  const recoveryCode = cleanRecoveryCode(body.recoveryCode);
  if (!id || !recoveryCode) return { ok: false, statusCode: 400, error: "Enter the request ID and recovery code." };

  const key = BETA_REQUEST_PREFIX + id;
  const record = await kvGetJSON(kv, key);
  if (!record || !(await hasRecoveryCode(record, recoveryCode))) {
    return { ok: false, statusCode: 403, error: "Request ID or recovery code is incorrect." };
  }

  const claimToken = token("rq_");
  await appendRequestClaim(record, claimToken);
  record.recoveredAt = isoNow();
  record.updatedAt = record.recoveredAt;
  await kvPutJSON(kv, key, record);

  const response = await deliverRequestStatus(kv, key, record);
...
async function betaAdminRequests(kv) {
  ...
  do {
    ...
    for (const item of listed.keys || []) {
      ...
      if (rec) requests.push(publicBetaRequest(rec, true));
    }
  } while (cursor);
...
async function betaAdminRequestUpdate(kv, body, admin) {
  const id = cleanRequestId(body.id);
  ...
  const next = ["new","reviewed","approved","denied"].includes(String(body.status)) ? String(body.status) : rec.status;
  rec.status = next;
  rec.updatedAt = isoNow();
  rec.updatedBy = admin.email;
  if (next === "reviewed") rec.reviewedAt = rec.reviewedAt || rec.updatedAt;
  if (next === "denied") {
    rec.deniedAt = rec.updatedAt;
    await archiveRequestAudit(kv, rec, admin, "denied", body.reason || "Denied by admin");
  }
  await kvPutJSON(kv, key, rec);
  return { ok: true, request: publicBetaRequest(rec, true) };
}

async function betaAdminRequestApprove(kv, body, admin) {
  const id = cleanRequestId(body.id || body.requestId);
  if (!id) return { ok: false, error: "Missing request id." };

  const requestKey = BETA_REQUEST_PREFIX + id;
  const rec = await kvGetJSON(kv, requestKey);
  if (!rec) return { ok: false, error: "Access request not found." };

  let code = cleanBetaCode(rec.approvedCode);
  if (code) {
    const existing = await kvGetJSON(kv, BETA_CODE_PREFIX + code);
    if (!existing) code = "";
  }
  if (!code) code = await findCodeCreatedForRequest(kv, id);

  if (!code) {
    const created = await betaAdminCreate(kv, {
      assignedUsername: rec.apexUsername,
      platform: rec.platform || "Auto",
      role: String(body.role || "tester"),
      badge: String(body.badge || "Original Beta Tester"),
      founderBadge: betaBool(body.founderBadge, false),
      maxDevices: clampInt(body.maxDevices, 1, 10, 2),
      notes: `Created from request ${id}${rec.mainLegend ? ` | Main: ${rec.mainLegend}` : ""}${rec.device ? ` | Device: ${rec.device}` : ""}`
    }, admin);
  ...
  const now = isoNow();
  rec.status = "approved";
  rec.approvedCode = code;
  rec.approvedAt = rec.approvedAt || now;
  rec.updatedAt = now;
  rec.updatedBy = admin.email;
  await kvPutJSON(kv, requestKey, rec);

  return {
    ok: true,
    code,
    request: publicBetaRequest(rec, true),
    message: "Approved. The requesting browser can now retrieve and unlock with this key automatically."
  };
}
...
async function betaAdminRequestDelete(kv, body, admin) {
  ...
  await archiveRequestAudit(kv, rec, admin, "deleted", body.reason || "Deleted from admin");
  await kv.delete(key);
  return {
    ok: true,
    deleted: id,
    auditRetainedDays: Math.floor(BETA_REQUEST_AUDIT_TTL / 86400),
    updatedBy: admin.email
  };
}

async function betaAdminRequestBlock(kv, body, admin) {
  const id = cleanRequestId(body.id);
  if (!id) return { ok: false, error: "Missing request id." };
  const key = BETA_REQUEST_PREFIX + id;
  const rec = await kvGetJSON(kv, key);
  ...
  const usernameHash = security.usernameHash || await protectedRequestHash(
    { APEXIQ_SECURITY_PEPPER: body.securityPepper || "", APEXIQ_ADMIN_SECRET: "" },
    "username",
    rec.apexUsernameNorm || normUsername(rec.apexUsername)
  ).catch(() => "");
  const reason = String(body.reason || "Blocked from suspicious access request").slice(0, 300);
  const blocks = [];

  // New v134 requests include all three hashes. Legacy requests can still
  // be blocked by username.
  const usernameValueHash = security.usernameHash || await sha256Hex(
    `legacy-username|${rec.apexUsernameNorm || normUsername(rec.apexUsername)}`
  );
  const usernameBlock = await putRequestBlock(
    kv,
    "username",
    usernameValueHash,
    rec.apexUsername || "Unknown username",
    admin,
    reason
  );
  if (usernameBlock) blocks.push(usernameBlock);

  const ipBlock = await putRequestBlock(kv, "ip", security.ipHash, "Hashed IP source", admin, reason);
  if (ipBlock) blocks.push(ipBlock);
  const deviceBlock = await putRequestBlock(kv, "device", security.deviceHash, "Hashed browser/device", admin, reason);
  if (deviceBlock) blocks.push(deviceBlock);

  await archiveRequestAudit(kv, rec, admin, "blocked-and-deleted", reason);
  await kv.delete(key);
  ...
  return {
    ...
    blocks,
    blockDays: Math.floor(BETA_REQUEST_BLOCK_TTL / 86400),
    auditRetainedDays: Math.floor(BETA_REQUEST_AUDIT_TTL / 86400),
    updatedBy: admin.email
  };
}

async function betaAdminBlocks(kv) {
  const blocks = [];
  let cursor;
  do {
    const listed = await kv.list({ prefix: BETA_REQUEST_BLOCK_PREFIX, cursor, limit: 1000 });
    cursor = listed.cursor;
    for (const item of listed.keys || []) {
      const rec = await kvGetJSON(kv, item.name);
      if (rec) blocks.push({ ...rec, key: item.name });
    }
  } while (cursor);
  blocks.sort((a, b) => String(b.createdAt || "").localeCompare(String(a.createdAt || "")));
  return { ok: true, blocks, count: blocks.length };
}

async function betaAdminUnblock(kv, body, admin) {
  const key = String(body.key || "").trim();
  if (!key.startsWith(BETA_REQUEST_BLOCK_PREFIX)) {
    return { ok: false, error: "Invalid block key." };
  }
  await kv.delete(key);
  return { ok: true, unblocked: key, updatedBy: admin.email };
}

async function betaAdminSecurityStatus(kv, env) {
  const blocks = await betaAdminBlocks(kv);
  let auditCount = 0;
  let cursor;
  do {
    const listed = await kv.list({ prefix: BETA_REQUEST_AUDIT_PREFIX, cursor, limit: 1000 });
    cursor = listed.cursor;
    auditCount += (listed.keys || []).length;
  } while (cursor);
  ...
  return {
    ...
    configured: !!(
      env.APEXIQ_TURNSTILE_SITE_KEY &&
      env.APEXIQ_TURNSTILE_SECRET_KEY &&
      requestSecurityPepper(env)
    ),
    turnstileSiteKeySet: !!env.APEXIQ_TURNSTILE_SITE_KEY,
    turnstileSecretSet: !!env.APEXIQ_TURNSTILE_SECRET_KEY,
    securityPepperSet: !!requestSecurityPepper(env),
    expectedHostname: String(env.APEXIQ_TURNSTILE_HOSTNAME || "imissanyway.github.io"),
    limits: BETA_REQUEST_LIMITS,
    blockCount: blocks.count,
    auditCount,
    auditRetentionDays: Math.floor(BETA_REQUEST_AUDIT_TTL / 86400),
    blockRetentionDays: Math.floor(BETA_REQUEST_BLOCK_TTL / 86400)
  };
}

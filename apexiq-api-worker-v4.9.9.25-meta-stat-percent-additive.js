...
const VERSION = "4.9.9.25-meta-stat-percent-additive";
...
function sanitizeLegendCards(cards) {
  ...
  for (const c of cards || []) {
    ...
    rows.push({
      legend,
      stats,
      trackers: stats.map(s => ({ name: s.label, key: s.label, value: s.value, source: s.source, percentile: s.percentile, topPercent: s.topPercent, rank: s.rank, total: s.total })),
      badges,
...
function extractALSTrackers(selected, raw) {
  ...
  function add(t) {
    ...
    out.push({
      name: String(name),
      key: String(first(t.key, name)),
      value,
      percentile: first(t.percentile, t.rankPercentile, t.rankPercent, t.topPercent, t.percent),
      topPercent: first(t.topPercent, t.percentile, t.rankPercentile, t.rankPercent, t.percent),
      rank: first(t.rank, t.globalRank, t.rankPosition, t.position, t.leaderboardRank),
      total: first(t.totalPlayers, t.total, t.population, t.totalCount)
    });
  }
...
function mergeLegendRows(a, b) {
  ...
  for (const row of [...(a || []), ...(b || [])]) {
    ...
    const stats = normalizeStats([...(existing.stats || []), ...(row.stats || []), ...(row.trackers || []).map(t => ({ label: t.name || t.key, value: t.value, source: t.source, percentile: t.percentile, topPercent: t.topPercent, rank: t.rank, total: t.total }))]);
    const badges = normalizeBadges([...(existing.badges || []), ...(existing.officialBadges || []), ...(row.badges || []), ...(row.officialBadges || [])]);
    by.set(key, { ...existing, ...row, stats, trackers: stats.map(s => ({ name: s.label, key: s.label, value: s.value, source: s.source, percentile: s.percentile, topPercent: s.topPercent, rank: s.rank, total: s.total })), badges, officialBadges: badges });
  }
...
function normalizeStats(stats) {
  ...
  for (const s of stats || []) {
    ...
    out.push({
      label,
      value: isNumeric(value) ? toNum(value) : value,
      source: s.source || "live",
      percentile: first(s.percentile, s.rankPercentile, s.rankPercent, s.topPercent, s.percent),
      topPercent: first(s.topPercent, s.percentile, s.rankPercentile, s.rankPercent, s.percent),
      rank: first(s.rank, s.globalRank, s.rankPosition, s.position, s.leaderboardRank),
      total: first(s.totalPlayers, s.total, s.population, s.totalCount)
    });
  }
  return out;
}
...
function parsePickRatesText(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    const matchName = LEGEND_NAMES.find(n => n.toLowerCase() === line.toLowerCase());
    ...
    let avgRank = "";
    let avgLevel = "";
    const windowLines = lines.slice(i + 1, Math.min(lines.length, i + 18));
    const joined = windowLines.join(" ");

    const pct = windowLines.find(l => /^(\d+(?:\.\d+)?)%$/.test(l)) || "";
    if (pct) pickRate = pct.replace("%", "");
    const tr = windowLines.find(l => /^[▲▼=]\s*\d/.test(l)) || windowLines.find(l => /(up|down|stable)/i.test(l) && /\d/.test(l)) || "";
    if (tr) trend = tr;

    let rankMatch =
      joined.match(/Avg\s*rank\s*([A-Za-z]+\s*\d?)/i) ||
      joined.match(/Average\s*rank\s*([A-Za-z]+\s*\d?)/i);
    if (rankMatch) avgRank = ("Avg rank " + rankMatch[1].trim()).replace(/\s+/g, " ");

    const lvlMatch = joined.match(/\b(?:level|lvl)\s*(\d{1,5})\b/i);
    if (lvlMatch) avgLevel = lvlMatch[1];

    // ALS text can split "Avg rank" and "Platinum 3 | level 837" across separate text nodes.
    if (!avgRank) {
      const idx = windowLines.findIndex(l => /Avg\s*rank/i.test(l));
      if (idx >= 0) {
        const near = windowLines.slice(idx, idx + 5).join(" ");
        const m = near.match(/\b(Bronze|Silver|Gold|Platinum|Diamond|Master|Apex Predator|Predator)\s*([1-4])?/i);
        if (m) avgRank = ("Avg rank " + m[0]).replace(/\s+/g, " ");
        const lm = near.match(/\b(?:level|lvl)\s*(\d{1,5})\b/i);
        if (!avgLevel && lm) avgLevel = lm[1];
      }
    }

    if (pickRate && !rows.some(r => r.legend === matchName)) {
      rows.push({
        legend: matchName,
        name: matchName,
        pickRate,
...
function fallbackPickratesMeta() {
  return LEGEND_NAMES.map((legend, i) => ({
    legend,
    name: legend,
    pickRate: "0",
    rate: "0",
    trend: "—",
    avgRank: "Waiting for ALS meta",
    avgLevel: "",

...
const VERSION = "4.9.9.22-strict-owned-fix";
...
function sanitizeLegendRowsStrict(rows) {
  ...
  for (const row of rows || []) {
    ...
    for (const b of [...(row.badges || []), ...(row.officialBadges || [])]) {
      ...
      else if (!isAmbiguousCollectionBadgeGuess(b) && (b.type || b.id || b.name || b.label || b.src)) entry.badges.push({ ...b, legend: b.legend || legend, verified: b.verified !== false, trust: b.trust || "legend_verified" });
    }
...
function isAmbiguousCollectionBadgeGuess(b) {
  const rawName = typeof b === "string" ? b : first(b && b.rawName, b && b.name, b && b.label, b && b.title, b && b.id, b && b.type, "");
  const src = typeof b === "string" ? "" : first(b && b.src, b && b.url, b && b.icon, b && b.image, "");
  const type = typeof b === "string" ? "" : first(b && b.type, b && b.badgeType, b && b.id, "");
  const text = String(rawName || "").toLowerCase().replace(/[_-]+/g, " ");
  const all = `${text} ${String(src || "").toLowerCase()} ${String(type || "").toLowerCase()}`;
  if (/\b(wake|wrath)\b/.test(text)) return false;
  if (/^(20|20b|20 bomb|20 kill|twenty kill|4k|4 k|4000|4,000|hammer)$/i.test(String(rawName || "").trim())) return true;
  if (/20b_collection|4k_collection|twenty|four_k|20kill|20_kill|4k|4_k|4000|hammer/.test(all)) return true;
  return false;
}
...
function normalizeBadges(badges) {
  ...
  for (const b of badges || []) {
    if (isAmbiguousCollectionBadgeGuess(b)) continue;
    const rawName = typeof b === "string" ? b : first(b.name, b.label, b.title, b.id, b.type, "");
    const src = typeof b === "string" ? "" : first(b.src, b.url, b.icon, b.image, "");
    const type = badgeIdFromStrongCollectionName(rawName) || badgeIdFromName(rawName || (src ? "unknown ALS badge slot" : ""));
    let name = rawName ? String(rawName) : "";
    if (!name && src) name = "Unknown ALS badge slot";
    if (!name) continue;
    const id = type || badgeIdFromName(name);
...
function badgeIdFromStrongCollectionName(name) {
  ...
  // Strict ownership: only exact ALS badge-family words become 20B/4K.
  // Bare 20B/4K/20 kill/4000/hammer strings are often visual guesses and are not trusted.
  if (/\bwake\b/.test(s)) return "twenty_kill";
  if (/\bwrath\b/.test(s)) return "four_k";
  return "";
}
...
function badgeTypeFromName(name) {
  const s = norm(name);
  if (/wake/.test(s)) return "twenty_kill";
  if (/wrath/.test(s)) return "four_k";
...
function parsePickRatesText(text) {
  const names = new Set(LEGEND_NAMES.map(x => x.toLowerCase()));
  const lines = String(text || "").split(/\n+/).map(s => s.trim()).filter(Boolean);
  const rows = [];
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    ...
    for (let j = i + 1; j < Math.min(lines.length, i + 8); j++) {
      const l = lines[j];
      if (!pickRate && /^(\d+(?:\.\d+)?)%$/.test(l)) pickRate = RegExp.$1;
      if (!trend && /^[▲▼=]\s*\d/.test(l)) trend = l;
      if (!avgRank && /Avg rank/i.test(l)) {
        avgRank = l.replace(/\s*\|\s*level\s*\d+.*/i, "").trim();
        const m = l.match(/\|\s*level\s*(\d+)/i);
        if (m) avgLevel = m[1];
      }
    }
    if (pickRate && !rows.some(r => r.legend === matchName)) {
      rows.push({ legend: matchName, name: matchName, pickRate, rate: pickRate, trend, avgRank, avgLevel, source: "ALS public legend pick-rates HTML" });
    }
  }

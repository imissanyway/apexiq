...
const VERSION = "4.9.9.17-accuracy-fix";
...
function badgeIdFromStrongCollectionName(name) {
  ...
  if (/\bwake\b|20\s*(kill|bomb)|twenty\s*kill/.test(s)) return "twenty_kill";
  if (/\bwrath\b|4\s*k\s*damage|4\s*k\s*badge|4000\s*damage|4,000\s*damage|four\s*thousand\s*damage|hammer/.test(s)) return "four_k";
  return "";
}

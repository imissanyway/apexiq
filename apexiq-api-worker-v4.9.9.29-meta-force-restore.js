...
const VERSION = "4.9.9.29-meta-force-restore";
...
function parsePickRatesText(text) {
  ...
  for (const legend of LEGEND_NAMES) {
    ...
    let m;
    while ((m = re.exec(compact))) namePositions.push({ legend, idx: m.index });
  }
  namePositions.sort((a,b) => a.idx - b.idx);

  function parseBand(legend, band) {
    const pct = band.match(/\b(\d+(?:\.\d+)?)\s*%/);
    if (!pct) return null;
    const trend = (band.match(/([▲▼]\s*\d+(?:\.\d+)?\s*%)/) || band.match(/\b(up|down|stable)\s*[\d.]*\s*%?/i) || ["",""])[1] || "";
    let avgRank = "";
    let avgLevel = "";
    const rankBlock = band.match(/Avg\s*rank\s*(.*?)(?:\blevel\b|$)/i);
    if (rankBlock) {
      const rank = rankBlock[1].match(/\b(Bronze|Silver|Gold|Platinum|Diamond|Master|Apex Predator|Predator)\s*([1-4])?/i);
      if (rank) avgRank = ("Avg rank " + rank[0]).replace(/\s+/g, " ").trim();
    }
    if (!avgRank) {
      const rank = band.match(/\b(Bronze|Silver|Gold|Platinum|Diamond|Master|Apex Predator|Predator)\s*([1-4])?\s*\|?\s*(?:level|lvl)\b/i);
      if (rank) avgRank = ("Avg rank " + rank[0].replace(/\|\s*(?:level|lvl)$/i, "")).replace(/\s+/g, " ").trim();
    }
    ...
    return {
      ...
      level: avgLevel,
      source: "ALS public legend pick-rates HTML"
    };
  }

  for (let i = 0; i < namePositions.length; i++) {
    const { legend, idx } = namePositions[i];
    if (rows.some(r => r.legend === legend)) continue;
    const end = namePositions[i + 1] ? namePositions[i + 1].idx : Math.min(compact.length, idx + 900);
    const band = compact.slice(idx, Math.max(end, idx + 160));
    const row = parseBand(legend, band);
  ...
  if (!rows.length) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const row = parseBand(legend, lines.slice(i, i + 20).join(" "));
      if (row) rows.push(row);
    }
  }

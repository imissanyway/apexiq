...
const VERSION = "4.9.9.82-worker-meta-28-row-parser";
...
async function handlePlayer(url, env, ctx, request) {
  ...
  if (lookup.uid) {
    ...
    bridge = env.ALS_API_KEY ? await fetchBestBridgeSmart(player, lookup.platform, env.ALS_API_KEY) : { ok: false, error: "ALS_API_KEY not set", platformUsed: lookup.platform === "auto" ? "PC" : lookup.platform, raw: {} };
    resolver.attempts.push({ step: "bridge_name_smart_parallel", ok: !!bridge.ok, platformUsed: bridge.platformUsed, resolvedPlayer: bridge.resolvedPlayer || player, error: bridge.error || "" });
  }
...
async function handleMeta(url, env, ctx, request) {
  const force = url.searchParams.get("refresh") === "1" || url.searchParams.get("nocache") === "1";
  const cacheUrl = new URL(url.toString());
  for (const key of ["refresh", "nocache", "_", "callback", "client"]) cacheUrl.searchParams.delete(key);
  const cacheKey = new Request(cacheUrl.toString(), request);
  if (!force) {
    const cached = await caches.default.match(cacheKey);
    if (cached) return cached.json();
  ...
  try {
    live = await fetchAlsPickRates(env, { browserFallback: force || url.searchParams.get("render") === "1" });
  } catch (e) {
    live = { ok: false, pickrates: [], source: "", attempts: [], error: String(e && e.message ? e.message : e) };
  }

  const rows = normalizeMetaRows(live.pickrates || []);
  const out = {
    ok: rows.length > 0,
    version: VERSION,
    source: live.source || (rows.length ? "ALS public legend pick-rates" : "No live meta rows available"),
    sourceKind: rows.length ? "live" : "empty",
    error: live.error || "",
    fetchedAt: new Date().toISOString(),
    count: rows.length,
  ...
  const response = new Response(JSON.stringify(out, null, 2), { headers: corsHeaders("*", rows.length ? 300 : 45) });
  if (!force && rows.length) ctx.waitUntil(caches.default.put(cacheKey, response.clone()));
  return out;
}
...
async function fetchAlsPickRates(env, options = {}) {
  ...
  // If the static parse is incomplete and Browser Rendering is configured, try rendered content once.
...
function parsePickRatesLines(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!legend) continue;
    const pctLine = lines.slice(i + 1, i + 5).find(l => /\b\d+(?:\.\d+)?\s*%\b/.test(l));
    if (!pctLine) continue;
    const pct = (pctLine.match(/\b(\d+(?:\.\d+)?)\s*%\b/) || [])[1];
    if (!pct) continue;
...
function parsePickRatesText(text) {
  ...
  for (let i = 0; i < positions.length; i++) {
    ...
    const pct = (band.match(/\b(\d+(?:\.\d+)?)\s*%\b/) || [])[1];
    if (!pct) continue;
...
function fallbackPickratesMeta() {
  return [{"legend": "Seer", "name": "Seer", "pickRate": "12.6", "rate": "12.6", "trend": "▲2830.74%", "avgRank": "Platinum 3", "averageRank": "Platinum 3", "avgLevel": "837", "averageLevel": "837", "level": "837", "source": "ApexIQ ALS visible snapshot fallback", "order": 1}, {"legend": "Axle", "name": "Axle", "pickRate": "12.1", "rate": "12.1", "trend": "▼29.85%", "avgRank": "Platinum 3", "averageRank": "Platinum 3", "avgLevel": "833", "averageLevel": "833", "level": "833", "source": "ApexIQ ALS visible snapshot fallback", "order": 2}, {"legend": "Pathfinder", "name": "Pathfinder", "pickRate": "9.8", "rate": "9.8", "trend": "▲333.24%", "avgRank": "Platinum 4", "averageRank": "Platinum 4", "avgLevel": "823", "averageLevel": "823", "level": "823", "source": "ApexIQ ALS visible snapshot fallback", "order": 3}, {"legend": "Octane", "name": "Octane", "pickRate": "6.5", "rate": "6.5", "trend": "▼6.44%", "avgRank": "Gold 2", "averageRank": "Gold 2", "avgLevel": "547", "averageLevel": "547", "level": "547", "source": "ApexIQ ALS visible snapshot fallback", "order": 4}, {"legend": "Ballistic", "name": "Ballistic", "pickRate": "5.8", "rate": "5.8", "trend": "▲882.72%", "avgRank": "Platinum 4", "averageRank": "Platinum 4", "avgLevel": "802", "averageLevel": "802", "level": "802", "source": "ApexIQ ALS visible snapshot fallback", "order": 5}, {"legend": "Valkyrie", "name": "Valkyrie", "pickRate": "5.1", "rate": "5.1", "trend": "▼51.92%", "avgRank": "Gold 1", "averageRank": "Gold 1", "avgLevel": "626", "averageLevel": "626", "level": "626", "source": "ApexIQ ALS visible snapshot fallback", "order": 6}, {"legend": "Wraith", "name": "Wraith", "pickRate": "4.8", "rate": "4.8", "trend": "▼4.66%", "avgRank": "Gold 3", "averageRank": "Gold 3", "avgLevel": "537", "averageLevel": "537", "level": "537", "source": "ApexIQ ALS visible snapshot fallback", "order": 7}, {"legend": "Bangalore", "name": "Bangalore", "pickRate": "4.7", "rate": "4.7", "trend": "▼15.16%", "avgRank": "Gold 2", "averageRank": "Gold 2", "avgLevel": "564", "averageLevel": "564", "level": "564", "source": "ApexIQ ALS visible snapshot fallback", "order": 8}, {"legend": "Mad Maggie", "name": "Mad Maggie", "pickRate": "4.7", "rate": "4.7", "trend": "▼39.66%", "avgRank": "Platinum 4", "averageRank": "Platinum 4", "avgLevel": "692", "averageLevel": "692", "level": "692", "source": "ApexIQ ALS visible snapshot fallback", "order": 9}, {"legend": "Conduit", "name": "Conduit", "pickRate": "4.3", "rate": "4.3", "trend": "—", "avgRank": "—", "averageRank": "—", "avgLevel": "—", "averageLevel": "—", "level": "—", "source": "ApexIQ ALS visible snapshot fallback", "order": 10}, {"legend": "Lifeline", "name": "Lifeline", "pickRate": "3.1", "rate": "3.1", "trend": "—", "avgRank": "—", "averageRank": "—", "avgLevel": "—", "averageLevel": "—", "level": "—", "source": "ApexIQ ALS visible snapshot fallback", "order": 11}, {"legend": "Ash", "name": "Ash", "pickRate": "2.8", "rate": "2.8", "trend": "—", "avgRank": "—", "averageRank": "—", "avgLevel": "—", "averageLevel": "—", "level": "—", "source": "ApexIQ ALS visible snapshot fallback", "order": 12}, {"legend": "Alter", "name": "Alter", "pickRate": "", "rate": "", "trend": "—", "avgRank": "—", "averageRank": "—", "avgLevel": "—", "averageLevel": "—", "level": "—", "source": "full roster fallback", "order": 13}, {"legend": "Bloodhound", "name": "Bloodhound", "pickRate": "", "rate": "", "trend": "—", "avgRank": "—", "averageRank": "—", "avgLevel": "—", "averageLevel": "—", "level": "—", "source": "full roster fallback", "order": 14}, {"legend": "Catalyst", "name": "Catalyst", "pickRate": "", "rate": "", "trend": "—", "avgRank": "—", "averageRank": "—", "avgLevel": "—", "averageLevel": "—", "level": "—", "source": "full roster fallback", "order": 15}, {"legend": "Caustic", "name": "Caustic", "pickRate": "", "rate": "", "trend": "—", "avgRank": "—", "averageRank": "—", "avgLevel": "—", "averageLevel": "—", "level": "—", "source": "full roster fallback", "order": 16}, {"legend": "Crypto", "name": "Crypto", "pickRate": "", "rate": "", "trend": "—", "avgRank": "—", "averageRank": "—", "avgLevel": "—", "averageLevel": "—", "level": "—", "source": "full roster fallback", "order": 17}, {"legend": "Fuse", "name": "Fuse", "pickRate": "", "rate": "", "trend": "—", "avgRank": "—", "averageRank": "—", "avgLevel": "—", "averageLevel": "—", "level": "—", "source": "full roster fallback", "order": 18}, {"legend": "Gibraltar", "name": "Gibraltar", "pickRate": "", "rate": "", "trend": "—", "avgRank": "—", "averageRank": "—", "avgLevel": "—", "averageLevel": "—", "level": "—", "source": "full roster fallback", "order": 19}, {"legend": "Horizon", "name": "Horizon", "pickRate": "", "rate": "", "trend": "—", "avgRank": "—", "averageRank": "—", "avgLevel": "—", "averageLevel": "—", "level": "—", "source": "full roster fallback", "order": 20}, {"legend": "Loba", "name": "Loba", "pickRate": "", "rate": "", "trend": "—", "avgRank": "—", "averageRank": "—", "avgLevel": "—", "averageLevel": "—", "level": "—", "source": "full roster fallback", "order": 21}, {"legend": "Mirage", "name": "Mirage", "pickRate": "", "rate": "", "trend": "—", "avgRank": "—", "averageRank": "—", "avgLevel": "—", "averageLevel": "—", "level": "—", "source": "full roster fallback", "order": 22}, {"legend": "Newcastle", "name": "Newcastle", "pickRate": "", "rate": "", "trend": "—", "avgRank": "—", "averageRank": "—", "avgLevel": "—", "averageLevel": "—", "level": "—", "source": "full roster fallback", "order": 23}, {"legend": "Rampart", "name": "Rampart", "pickRate": "", "rate": "", "trend": "—", "avgRank": "—", "averageRank": "—", "avgLevel": "—", "averageLevel": "—", "level": "—", "source": "full roster fallback", "order": 24}, {"legend": "Revenant", "name": "Revenant", "pickRate": "", "rate": "", "trend": "—", "avgRank": "—", "averageRank": "—", "avgLevel": "—", "averageLevel": "—", "level": "—", "source": "full roster fallback", "order": 25}, {"legend": "Sparrow", "name": "Sparrow", "pickRate": "", "rate": "", "trend": "—", "avgRank": "—", "averageRank": "—", "avgLevel": "—", "averageLevel": "—", "level": "—", "source": "full roster fallback", "order": 26}, {"legend": "Vantage", "name": "Vantage", "pickRate": "", "rate": "", "trend": "—", "avgRank": "—", "averageRank": "—", "avgLevel": "—", "averageLevel": "—", "level": "—", "source": "full roster fallback", "order": 27}, {"legend": "Wattson", "name": "Wattson", "pickRate": "", "rate": "", "trend": "—", "avgRank": "—", "averageRank": "—", "avgLevel": "—", "averageLevel": "—", "level": "—", "source": "full roster fallback", "order": 28}];
}

...
const VERSION = "4.9.9.89-worker-beta-admin-kv";
    ...
    try {
      ...
      if (url.pathname === "/api/health") {
        return json({
          ...
          rollbackSafe: true,
          stableBase: "v4.6.8",
          endpoints: ["/api/player","/api/ranked-fast","/api/ranked","/api/meta","/api/beta/verify","/api/beta/session","/api/beta/admin/login","/api/beta/admin/users","/api/rendered-profile","/api/badge-scan","/api/rendered-ranked","/api/doctor","/api/als-test","/api/switch-ranked","/api/health"]
        }, origin);
      }
...
async function routeBetaAccess(request, env, origin, url) {
  ...
  try {
    ...
    if (url.pathname.startsWith("/api/beta/admin/")) {
      ...
      if (!admin.ok) return json(admin, origin, 401);

      if (url.pathname === "/api/beta/admin/users" && request.method === "GET") {
        return json(await betaAdminUsers(kv), origin, 200);
      }
      if (url.pathname === "/api/beta/admin/create" && request.method === "POST") {
        return json(await betaAdminCreate(kv, body, admin), origin, 200);
      }
      if (url.pathname === "/api/beta/admin/update" && request.method === "POST") {
        return json(await betaAdminUpdate(kv, body, admin), origin, 200);
      }
      if (url.pathname === "/api/beta/admin/reset-devices" && request.method === "POST") {
        return json(await betaAdminResetDevices(kv, body, admin), origin, 200);
      }
      if (url.pathname === "/api/beta/admin/disable" && request.method === "POST") {
        return json(await betaAdminUpdate(kv, { ...body, status: "disabled" }, admin), origin, 200);
      }
      if (url.pathname === "/api/beta/admin/enable" && request.method === "POST") {
        return json(await betaAdminUpdate(kv, { ...body, status: "active" }, admin), origin, 200);
      }
      if (url.pathname === "/api/beta/admin/delete" && request.method === "POST") {
        return json(await betaAdminDelete(kv, body, admin), origin, 200);
      }
    }
...
function normUsername(input) { return cleanUsername(input).toLowerCase(); }
...
async function betaVerify(kv, body) {
  ...
  return { ok: true, sessionToken, expiresAt, code, assignedUsername: record.assignedUsername, platform: record.platform || "Auto", role: record.role || "tester", badge: record.badge || "Original Beta Tester", maxDevices, devicesUsed: devices.length, admin: record.role === "admin" || record.role === "founder" };
}
...
async function betaAdminDelete(kv, body, admin) {
  const code = cleanBetaCode(body.code);
  if (!code) return { ok: false, error: "Missing code." };
  await kv.delete(BETA_CODE_PREFIX + code);
...
async function handleMeta(url, env, ctx, request) {
  ...
  if (!rows.length) {
    rows = normalizeMetaRows(fallbackPickratesMeta());
  }
  const usedFallback = !liveRows && rows.length;
  const out = {
    ok: rows.length > 0,
    version: VERSION,
    source: usedFallback ? "ALS public legend pick-rates snapshot fallback" : (live.source || "ALS public legend pick-rates"),
    sourceKind: usedFallback ? "snapshot-fallback" : "live",
    fallback: usedFallback,
    liveOk: liveRows > 0,
    liveError,
    error: rows.length ? "" : (liveError || "No live meta rows available"),
    fetchedAt: new Date().toISOString(),
    count: rows.length,
...
function fallbackPickratesMeta() {
  return [
  {
    ...
    "name": "Seer",
    "rate": "13.3",
    "averageRank": "Platinum 3",
    "averageLevel": "777",
    "level": "777",
    "source": "ALS public legend pick-rates snapshot",

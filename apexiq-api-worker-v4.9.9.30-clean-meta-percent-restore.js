...
const VERSION = "4.9.9.30-clean-meta-percent-restore";
    ...
    try {
      if (url.pathname === "/api/health") {
        return json({
          ...
          rollbackSafe: true,
          stableBase: "v4.6.8",
          endpoints: ["/api/player","/api/ranked-fast","/api/ranked","/api/meta","/api/ranked?tracker=1","/api/rendered-profile","/api/badge-scan","/api/rendered-ranked","/api/doctor","/api/als-test","/api/switch-ranked","/api/health"]
        }, origin);
      }
...
function respond(url, data, origin = "*", status = 200) {
  ...
  return json(data, origin, status);
}
...
async function fetchJson(url, options = {}) {
  const res = await fetch(url, {
    ...
    headers: {
      ...
      "User-Agent": "ApexIQ/4.9.9.6 (+Cloudflare Worker)",
      ...(options.headers || {})
    }
  });
...
async function fetchText(url, options = {}) {
  const res = await fetch(url, {
    ...
    headers: {
      ...
      "User-Agent": "Mozilla/5.0 ApexIQ/4.9.9.6 recovery",
      ...(options.headers || {})
    }
  });
  const text = await res.text();
  ...
  for (const platform of order) {
    for (const mode of ["uid", "player"]) {
      ...
      try {
        ...
        tried.push({ platform, mode, ok, score, error: ok ? "" : data.Error || data.error || "No payload" });
        if (ok) valid.push({ ok: true, source: "Apex Legends Status /bridge UID", platformUsed: platform, raw: data, score, resolver: "ALS UID bridge" });
      } catch (e) {
        tried.push({ platform, mode, ok: false, error: e.message });
      }
    }
  }
  if (valid.length) {
...
async function handlePlayerLite(url, env, ctx, request) {
  ...
  if (lookup.uid) {
    ...
    bridge = env.ALS_API_KEY ? await fetchBestBridgeSmart(player, lookup.platform, env.ALS_API_KEY) : { ok: false, error: "ALS_API_KEY not set", platformUsed: lookup.platform === "auto" ? "PC" : lookup.platform, raw: {} };
    resolver.attempts.push({ step: "lite_bridge_name_smart", ok: !!bridge.ok, platformUsed: bridge.platformUsed, resolvedPlayer: bridge.resolvedPlayer || player, error: bridge.error || "" });
  }
  ...
  const response = new Response(JSON.stringify(out, null, 2), { headers: corsHeaders("*") });
...
async function handlePlayer(url, env, ctx, request) {
  ...
  if (lookup.uid) {
    bridge = env.ALS_API_KEY ? await fetchBridgeByUid(lookup.uid, lookup.platform, env.ALS_API_KEY) : bridge;
    resolver.attempts.push({ step: "bridge_uid", ok: !!bridge.ok, platformUsed: bridge.platformUsed, error: bridge.error || "" });
  } else {
    bridge = env.ALS_API_KEY ? await fetchBestBridgeSmart(player, lookup.platform, env.ALS_API_KEY) : { ok: false, error: "ALS_API_KEY not set", platformUsed: lookup.platform === "auto" ? "PC" : lookup.platform, raw: {} };
    resolver.attempts.push({ step: "bridge_name_smart", ok: !!bridge.ok, platformUsed: bridge.platformUsed, resolvedPlayer: bridge.resolvedPlayer || player, error: bridge.error || "" });
  }
  ...
  // When normal ALS bridge does not find the Steam/display name, automatically search ALS
  // for a verified UID/profile match before falling back to ranked-only data.
  ...
  if (!bridgeHasUsefulData(bridge) && !uid && player) {
    ...
    if (uidLookup.ok && uidLookup.uid) {
      ...
      if (verified.exists) {
        ...
        resolver.attempts.push({ step: "bridge_from_verified_uid", ok: !!uidBridge.ok, platformUsed: uidBridge.platformUsed, error: uidBridge.error || "" });
  ...
  let rendered = { ok: false, error: "Browser Run not enabled.", legendCards: [], accountBadges: [] };
  if (browserReady(env)) {
    try { rendered = await getRenderedProfile(lookup.uid ? "" : resolvedPlayer, platform, uid, env); }
    catch (e) { rendered = { ok: false, error: e.message, legendCards: [], accountBadges: [] }; }
  }

  const prestigeProbe = rendered && rendered.ok ? rendered : await safePrestigeProbe(lookup.uid ? "" : resolvedPlayer, platform, uid, env);
  const tracker = await safeTracker(resolvedPlayer || player || uid, platform, env);
  let profile = buildProfile(resolvedPlayer || player || uid, lookup.platform, bridge, prestigeProbe, tracker, rendered);

  // If normal bridge is empty but rendered-by-UID worked, keep it as the actual profile.
  ...
  for (const platform of order) {
    ...
    try {
      ...
      if (ok) {
        ...
        tried.push({ platform, ok: true, score });
        valid.push({ ok: true, source: "Apex Legends Status /bridge", platformUsed: platform, raw: data, score });
      } else {
        tried.push({ platform, ok: false, error: data.Error || data.error || "No player payload returned." });
      }
    } catch (e) {
      tried.push({ platform, ok: false, error: e.message });
    }
  }
  if (valid.length) {
  ...
  for (const legend of LEGEND_NAMES) {
    ...
    if (best && (best.stats.length || best.badges.length)) {
      rows.push({
        legend,
        stats: best.stats,
        trackers: best.stats.map(st => ({ name: st.label, key: st.label, value: st.value, source: st.source, topPercent: st.topPercent, percentile: st.percentile, rank: st.rank })),
        badges: best.badges,
...
function parseStatsFromSegment(seg) {
  ...
  function add(label, value, tail = "") {
    const cleanLabel = String(label || "").replace(/\s+/g, " ").trim();
    const num = toNum(value);
    if (!cleanLabel || !num) return;
    if (/level|prestige|views|reputation|profile|badge|skin|frame|pose|intro/i.test(cleanLabel)) return;
    const key = `${cleanLabel.toLowerCase()}|${num}`;
    if (seen.has(key)) return;
    seen.add(key);
    const top = String(tail || "").match(/\bTop\s*(\d+(?:\.\d+)?)\s*%/i);
    const rank = String(tail || "").match(/#\s*([0-9][0-9,]{0,10})/);
    out.push({ label: cleanLabel, value: num, source: "Browser Run rendered ALS parser", topPercent: top ? top[1] : "", percentile: top ? top[1] : "", rank: rank ? rank[1] : "" });
  }
  ...
  for (const label of APEXIQ_RENDERED_STAT_LABELS) {
    ...
    let re = new RegExp(`${lab}\\s+([0-9][0-9,]{0,12})([\\s\\S]{0,180})`, "ig");
    let m;
    while ((m = re.exec(clean))) add(label, m[1], m[2]);

    re = new RegExp(`([0-9][0-9,]{0,12})\\s+${lab}([\\s\\S]{0,180})`, "ig");
    while ((m = re.exec(clean))) add(label, m[1], m[2]);
  }
...
function extractLegendStats(raw) {
  ...
  function addRow(legendName, node, sourceHint) {
    ...
    if (current) {
      ...
      current.trackers = current.stats.map(s => ({ name: s.label, key: s.label, value: s.value, source: s.source, topPercent: s.topPercent, percentile: s.percentile, rank: s.rank, total: s.total }));
      return;
    }
    out.push({
      legend,
  ...
  if (badgeWord === "wrath") {
    return {
      ...
      badge: {
        ...
        rawName: raw,
        value: n || value || "",
        verified: n >= 4 || n === 0 || value !== undefined,
        trust: "legend_verified",
        source: source || "Apex Legends Status badge tracker",
        note: n ? `ALS Wrath tier/value ${n}` : ""
      }
    };
  }
  return null;
...
function uniqueBadgesStrict(list) {
  ...
  for (const b of list || []) {
    ...
    const key = `${legend}|${type}|${norm(b.rawName || b.name || b.label || "")}`;
...
function uniqueVerifiedBadges(list) {
  const out = [], seen = new Set();
  for (const b of list || []) {
    const key = `${norm(b.legend)}|${b.type}|${norm(b.name)}|${b.src || ""}|${b.trust}`;
    if (seen.has(key)) continue;
    seen.add(key);
    out.push(b);
  }
...
async function handleMeta(url, env, ctx, request) {
  ...
  let error = "";
  try {
    const live = await fetchAlsPickRates();
    pickrates = live.pickrates;
    source = live.source;
  } catch (e) {
    error = e.message;
  }
  ...
  if (!pickrates.length) {
    pickrates = fallbackPickratesMeta();
    source = source || "ApexIQ fallback roster";
  }
...
async function fetchAlsPickRates() {
  const url = "https://apexlegendsstatus.com/game-stats/legends-pick-rates";
  const html = await fetchText(url, { headers: { "Accept": "text/html,*/*" } });
  const text = textFromHtml(html);
  const rows = parsePickRatesText(text);
  return { ok: rows.length > 0, source: "ALS public legend pick-rates HTML", url, pickrates: rows };
}
...
function parsePickRatesText(text) {
  ...
  function parseBand(legend, band) {
    const pct = String(band || "").match(/\b(\d+(?:\.\d+)?)\s*%/);
    if (!pct) return null;
    const trend = (band.match(/([▲▼]\s*\d+(?:\.\d+)?\s*%)/) || band.match(/\b(up|down|stable)\s*[\d.]*\s*%?/i) || ["",""])[1] || "";
    let avgRank = "";
    let avgLevel = "";
    const rank = band.match(/\b(Bronze|Silver|Gold|Platinum|Diamond|Master|Apex Predator|Predator)\s*([1-4])?\b/i);
    if (rank) avgRank = ("Avg rank " + rank[0]).replace(/\s+/g, " ").trim();
    const lvl = band.match(/\b(?:level|lvl)\s*(\d{1,5})\b/i);
    if (lvl) avgLevel = lvl[1];
    return { legend, name: legend, pickRate: pct[1], rate: pct[1], trend, avgRank, averageRank: avgRank, avgLevel, averageLevel: avgLevel, level: avgLevel, source: "ALS public legend pick-rates HTML" };
  }

  for (let i = 0; i < positions.length; i++) {
    const { legend, idx } = positions[i];
    if (rows.some(r => r.legend === legend)) continue;
    const next = positions[i + 1]?.idx || Math.min(compact.length, idx + 900);
    const row = parseBand(legend, compact.slice(idx, Math.max(next, idx + 180)));
  ...
  if (!rows.length) {
    const lines = String(text || "").split(/\n+/).map(s => s.trim()).filter(Boolean);
    for (let i = 0; i < lines.length; i++) {
      const legend = LEGEND_NAMES.find(n => n.toLowerCase() === lines[i].toLowerCase());
      if (!legend || rows.some(r => r.legend === legend)) continue;
      const row = parseBand(legend, lines.slice(i, i + 24).join(" "));
      if (row) rows.push(row);
    }
  }

  return rows.sort((a,b) => Number(b.pickRate) - Number(a.pickRate));
}

function fallbackPickratesMeta() {
  const snapshot = [{"legend": "Seer", "name": "Seer", "pickRate": "12.6", "rate": "12.6", "trend": "▲2830.74%", "avgRank": "Platinum 3", "averageRank": "Platinum 3", "avgLevel": "837", "averageLevel": "837", "level": "837", "source": "ApexIQ ALS visible snapshot fallback", "order": 1}, {"legend": "Axle", "name": "Axle", "pickRate": "12.1", "rate": "12.1", "trend": "▼29.85%", "avgRank": "Platinum 3", "averageRank": "Platinum 3", "avgLevel": "833", "averageLevel": "833", "level": "833", "source": "ApexIQ ALS visible snapshot fallback", "order": 2}, {"legend": "Pathfinder", "name": "Pathfinder", "pickRate": "9.8", "rate": "9.8", "trend": "▲333.24%", "avgRank": "Platinum 4", "averageRank": "Platinum 4", "avgLevel": "823", "averageLevel": "823", "level": "823", "source": "ApexIQ ALS visible snapshot fallback", "order": 3}, {"legend": "Octane", "name": "Octane", "pickRate": "6.5", "rate": "6.5", "trend": "▼6.44%", "avgRank": "Gold 2", "averageRank": "Gold 2", "avgLevel": "547", "averageLevel": "547", "level": "547", "source": "ApexIQ ALS visible snapshot fallback", "order": 4}, {"legend": "Ballistic", "name": "Ballistic", "pickRate": "5.8", "rate": "5.8", "trend": "▲882.72%", "avgRank": "Platinum 4", "averageRank": "Platinum 4", "avgLevel": "802", "averageLevel": "802", "level": "802", "source": "ApexIQ ALS visible snapshot fallback", "order": 5}, {"legend": "Valkyrie", "name": "Valkyrie", "pickRate": "5.1", "rate": "5.1", "trend": "▼51.92%", "avgRank": "Gold 1", "averageRank": "Gold 1", "avgLevel": "626", "averageLevel": "626", "level": "626", "source": "ApexIQ ALS visible snapshot fallback", "order": 6}, {"legend": "Wraith", "name": "Wraith", "pickRate": "4.8", "rate": "4.8", "trend": "▼4.66%", "avgRank": "Gold 3", "averageRank": "Gold 3", "avgLevel": "537", "averageLevel": "537", "level": "537", "source": "ApexIQ ALS visible snapshot fallback", "order": 7}, {"legend": "Bangalore", "name": "Bangalore", "pickRate": "4.7", "rate": "4.7", "trend": "▼15.16%", "avgRank": "Gold 2", "averageRank": "Gold 2", "avgLevel": "564", "averageLevel": "564", "level": "564", "source": "ApexIQ ALS visible snapshot fallback", "order": 8}, {"legend": "Mad Maggie", "name": "Mad Maggie", "pickRate": "4.7", "rate": "4.7", "trend": "▼39.66%", "avgRank": "Platinum 4", "averageRank": "Platinum 4", "avgLevel": "692", "averageLevel": "692", "level": "692", "source": "ApexIQ ALS visible snapshot fallback", "order": 9}, {"legend": "Conduit", "name": "Conduit", "pickRate": "4.3", "rate": "4.3", "trend": "—", "avgRank": "—", "averageRank": "—", "avgLevel": "—", "averageLevel": "—", "level": "—", "source": "ApexIQ ALS visible snapshot fallback", "order": 10}, {"legend": "Lifeline", "name": "Lifeline", "pickRate": "3.1", "rate": "3.1", "trend": "—", "avgRank": "—", "averageRank": "—", "avgLevel": "—", "averageLevel": "—", "level": "—", "source": "ApexIQ ALS visible snapshot fallback", "order": 11}, {"legend": "Ash", "name": "Ash", "pickRate": "2.8", "rate": "2.8", "trend": "—", "avgRank": "—", "averageRank": "—", "avgLevel": "—", "averageLevel": "—", "level": "—", "source": "ApexIQ ALS visible snapshot fallback", "order": 12}];
  const used = new Set(snapshot.map(r => norm(r.legend)));
  const rest = LEGEND_NAMES
    .filter(legend => !used.has(norm(legend)))
    .map((legend, i) => ({
      legend,
      name: legend,
      pickRate: "",
      ...
      averageLevel: "—",
      level: "—",
      source: "fallback roster",
      order: snapshot.length + i + 1
    }));

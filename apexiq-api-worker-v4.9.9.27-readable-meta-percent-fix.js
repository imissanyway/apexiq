...
const VERSION = "4.9.9.27-readable-meta-percent-fix";
...
function parseStatsFromSegment(seg) {
  ...
  function add(label, value, tail = "") {
    ...
    if (seen.has(key)) return;
    seen.add(key);
    const tailText = String(tail || "");
    const top = tailText.match(/\bTop\s*(\d+(?:\.\d+)?)\s*%/i);
    const rank = tailText.match(/#\s*([0-9][0-9,]{0,10})/);
    out.push({
      label: cleanLabel,
      value: num,
      source: "Browser Run rendered ALS parser",
      topPercent: top ? top[1] : "",
      percentile: top ? top[1] : "",
      rank: rank ? rank[1] : ""
    });
  }
  ...
  for (const label of APEXIQ_RENDERED_STAT_LABELS) {
    ...
    let re = new RegExp(`${lab}\\s+([0-9][0-9,]{0,12})([^A-Z]{0,220})`, "ig");
    let m;
    while ((m = re.exec(clean))) add(label, m[1], m[2]);

    re = new RegExp(`([0-9][0-9,]{0,12})\\s+${lab}([^A-Z]{0,220})`, "ig");
    while ((m = re.exec(clean))) add(label, m[1], m[2]);
  }
...
function sanitizeLegendCards(cards) {
  ...
  for (const c of cards || []) {
    ...
    rows.push({
      legend,
      stats,
      trackers: stats.map(s => ({ name: s.label, key: s.label, value: s.value, source: s.source, topPercent: s.topPercent || '', percentile: s.percentile || '', rank: s.rank || '', total: s.total || '' })),
      badges,
...
function mergeLegendRows(a, b) {
  ...
  for (const row of [...(a || []), ...(b || [])]) {
    ...
    by.set(key, { ...existing, ...row, stats, trackers: stats.map(s => ({ name: s.label, key: s.label, value: s.value, source: s.source, topPercent: s.topPercent || '', percentile: s.percentile || '', rank: s.rank || '', total: s.total || '' })), badges, officialBadges: badges });
  }
...
function parsePickRatesText(text) {
  ...
  const rows = [];
  function addRow(legend, band) {
    if (!legend || rows.some(r => r.legend === legend)) return;
    const pick = band.match(/\b(\d+(?:\.\d+)?)\s*%/);
    if (!pick) return;
    let trend = "";
    const t = band.match(/([▲▼]\s*\d+(?:\.\d+)?\s*%)/) || band.match(/\b(up|down|stable)\s*[\d.]*\s*%?/i);
    if (t) trend = String(t[1] || t[0] || "").trim();
    let avgRank = "";
    const rankNear = band.match(/Avg\s*rank\s*(.{0,80})/i);
    const rankSource = rankNear ? rankNear[1] : band;
    const rank = rankSource.match(/\b(Apex Predator|Predator|Master|Diamond|Platinum|Gold|Silver|Bronze)\s*([1-4])?/i);
    if (rank) avgRank = ("Avg rank " + rank[0]).replace(/\s+/g, " ").trim();
    const level = band.match(/\b(?:level|lvl)\s*(\d{1,5})\b/i);
    const avgLevel = level ? level[1] : "";
    rows.push({
      legend,
      name: legend,
      pickRate: pick[1],
  ...
  for (const legend of LEGEND_NAMES) {
    const escaped = escapeRegExp(legend);
    const re = new RegExp(`\\b${escaped}\\b`, "ig");
    let m;
    while ((m = re.exec(compact))) {
      addRow(legend, compact.slice(m.index, Math.min(compact.length, m.index + 700)));
      if (rows.some(r => r.legend === legend)) break;
    }
  }
  ...
  if (!rows.length) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (!legend) continue;
      addRow(legend, lines.slice(i, i + 20).join(" "));
    }
  }
